...
import { createTask, getTasks, updateTaskStatus, getTaskStatsByIntern, bulkAssignTasks } from '../controllers/taskController.js';
import {
  checkIn, checkOut, getMyAttendance, getAllAttendance, updateAttendance,
  markAttendanceForIntern, markSelfAttendance          // <-- new import
...
router.post('/bulk-assign',    requireRole('admin', 'manager'), bulkAssignTasks);
router.get('/',                getTasks);
router.get('/stats/by-intern', requireRole('admin', 'manager'), getTaskStatsByIntern);
router.patch('/:id/status',    updateTaskStatus);

// ─── Attendance ──────────────────────────────────────────────
router.post('/attendance/mark-self',  markSelfAttendance);               // <-- intern self‑mark
